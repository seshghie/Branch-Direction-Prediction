...
for filename in os.listdir(results_dir):
    for size in benchmarks_table:
        if f'shaheen_local_fixPC_{size}' in filename:
...
plt.ylabel("Accuracy (%)")
plt.title("Benchmark Branch Prediction Accuracy for Local Predictor w/ 10-bit fixed PC")
...
plt.tight_layout()
plt.savefig("branch_prediction_accuracy_sorted_local_fixPC.png")